...
<h1 class="document-type-label" style="text-align: center;">
    ...
    <?php _e( 'Fatura', 'woocommerce-pdf-invoices-packing-slips' ); ?>
</h1>
...
<table class="order-data-addresses" style="border: 1px dashed #ccc; background-color: #f2f2f2;">
    <tr>
	...
	?>
      ...
      <!--   <table class="order-data-addresses" style="border: 1px dashed #ccc; background-color: #f2f2f2;">
            <tr>
                <td class="address shipping-address" style="padding: 15px;">
				 <b><?php //_e( 'REPRESENTAÇÃO COMERCIAL', 'woocommerce-pdf-invoices-packing-slips' ); ?></b><br>
                  
                    <div class="billing-email">
                        <?php /* $email_fornecedor = implode(', ', (array) $this->order->get_meta('email_fornecedor') );
                              
                              $user = get_user_by( 'email', $email_fornecedor );
                              //echo '<b>' . $user->first_name . '</b><br>' . $user->last_name;
                              $billing_company = get_user_meta( $user->ID, 'billing_company', true );    
                              $billing_address_1 = get_user_meta( $user->ID, 'billing_address_1', true );
                              //$billing_address_2 = get_user_meta( $user->ID, 'billing_address_2', true );    
        ...
        <td class="order-data">
            <table style="padding-left: 5px;">
                ...
                <tr class="payment-method">
                    ...
                    <td style="background-color:#f2f2f2; padding: 3px;"><?php $this->payment_method(); ?></td>
                </tr>
...
<table style="width: 100%; margin-bottom: 20px;">
	...
	printf( '%s', esc_html( $current_user->user_firstname ) );
    $comissao_venda = implode(', ', (array) $this->order->get_meta('comissao_porcentagem') );
?>
        </td>
        <td style="border: 1px solid #c2c2c2; padding: 5px; text-align:center;">
 <?php 
	$comissao_venda = implode(', ', (array) $this->order->get_meta('comissao_porcentagem') );
     echo "COMISSÃO DE VENDA: R$ " . $comissao_venda;
?>
        </td>
    </tr>
</table>
...
<table style="width: 100%; margin-top: 20px; background-color: #F2FAF8; border: 1px dashed #ccc;">
    <tr>
        <td style="border: 1px solid #F2FAF8; width:50%; padding:5px;">
            <div class="document-notes">
                <?php do_action( 'wpo_wcpdf_before_document_notes', $this->type, $this->order ); ?>
                <?php if ( $this->get_document_notes() ) : ?>
        ...
        <td style="border: 1px solid #ddd; padding:5px; background-color:#F2FAF8;">
            <div class="document-notes">
            
            <?php
            //$notas_da_fatura = implode(',', (array) $this->order->get_meta('notas_da_fatura') );

            //echo '<pre>'.$notas_da_fatura.'</pre>';
            ?>
